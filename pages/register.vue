...
const handleRegister = async () => {
  ...
  try {
    ...
    if (error)
      throw error
    alert('Kiểm tra link login được gửi vào email của bạn ')
  }
  catch (error) {
    alert(error.message)
  }
  ...
  <section>
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
      <a href="#" class="flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white">
        <img class="w-8 h-8 mr-2" src="login.vue" alt="logo">
        Tu Tiên Giới
      </a>
        ...
        <div class="p-6 space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="handleRegister">
            ...
            <div>
              ...
              <input  v-model="rePassword" type="password" name="password" id="password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="">
            </div>
