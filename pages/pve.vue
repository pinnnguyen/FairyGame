...
const { data: war, pending } = await useAsyncData('war', () => $fetch('/api/war', {
  method: 'POST',
  body: {
    kind: 'solo',
    player: {
...
console.log('war', war.value)
</script>

<template>
  <div class="h-screen bg-white" v-if="!pending">
      ...
      <div class="flex justify-between p-2 pt-2">
        <div>
          <div class="flex items-center h-[100px] w-[120px]">
            ...
            <span class="pb-[2px] font-medium">{{ playerInfo.name }} [lv:{{ playerInfo.level }}]</span>
          </div>
          <div class="mt-2">
              ...
              <div class="h-3 w-24 rounded-full bg-[#212121] flex items-center p-[2px]">
                <div
                  class="h-2 w-20 rounded-full bg-red-600" :style="{
                    width: `${playerInfo?.attribute?.hp}px`,
                  }"
                />
              </div>
            </div>
          ...
          <div class="flex justify-start">
            Công kích:({{ playerInfo?.attribute?.damage }})<br>
            Phòng ngự:({{ playerInfo?.attribute?.def }})<br>
          </div>
        </div>
        <div>
          <div class="flex items-center h-[100px] w-[120px]">
<!--            <span class="pb-[2px] font-medium">Ma hóa chi kiếm ma [lv:{{ war.enemy?.level }}]</span>-->
...
<!--            Công kích:({{ war.enemy?.damage }})<br>-->
<!--            Phòng ngự:({{ war.enemy?.def }})<br>-->
          </div>
        </div>
      </div>
