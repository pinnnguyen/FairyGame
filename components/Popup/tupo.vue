  ...
  <div class="blocker" v-if="upgrade.condition.beUpgraded">
    <div class="bg-[#1d160e] p-3 transform-center text-white fixed border-2 border-[#dcc18d] z-[999] w-[90%] rounded sm:w-[640px] md:w-[640px] lg:w-[640px]">
      ...
      <div class="flex flex-col items-center justify-center mb-1">
        <p class="text-center text-[#dcc18d] text-sm font-bold mb-2 uppercase">
          Đột phá cảnh giới
        </p>
      </div>
      <p v-if="error" class="text-center text-red-600 text-xs mb-3 uppercase">
        {{ error }}
      </p>
      <p v-if="!res.status" class="text-center text-red-600 text-xs mb-3 uppercase">
        {{ res.message }}
      </p>
      <p v-if="res.status" class="text-center text-green-500 text-xs mb-3 uppercase">
        {{ res.message }}
      </p>
