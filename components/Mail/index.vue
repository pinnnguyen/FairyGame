...
<template>
  <var-popup v-model:show="toggle.mail">
    <div class="h-[40vh] w-[calc(100vw_-_70px)] m-auto bg-[#97c7e9] p-4 border-2 border-black rounded text-10 text-white/80">
      <div class="h-full p-2" style="background-image: linear-gradient(#244363, #6a8ca7)">
        ...
        <div v-if="readMail?.records.length > 0" class="relative mt-2">
          <div class="mb-10">
            Phần thưởng
          </div>
          <MailGems :gems="readMail.records" />
        </div>
      </div>
    </div>
  </var-popup>
  <div class="h-[55vh] w-[calc(100vw_-_70px)] m-auto bg-[#485384] p-2 border-2 border-black rounded">
    ...
    <div v-for="mail in mails" :key="mail?._id" class="flex items-center bg-[#87c6ea] p-2 m-1 gap-2 rounded" @click.stop="read(mail)">
      <icon name="material-symbols:mark-email-unread" size="30" />
      <div class="flex flex-col items-start">
        <div>{{ mail?.title }}</div>
        <div v-if="mail?.records.length > 0" class="text-10 text-green-500">
