...
export const handleEquipUpgrade = (io: any, socket: any, params: {
  _channel: string
}) => {
  socket.join(params._channel)
  socket.on('equip:upgrade:preview', async (_equipId: string) => {
    ...
    io.to(params._channel).emit('equip:preview:response', require)
  })
  ...
  socket.on('equip:upgrade', async (type: string, _equipId: string) => {
    ...
    io.to(params._channel).emit('equip:upgrade:response', {
      gold,
      cuongHoaThach,
