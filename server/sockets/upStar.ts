...
export const handleEquipStar = (io: any, socket: any) => {
  socket.on('equip:star:start', (_channel: string) => {
    socket.join(_channel)
    socket.on('equip:start:preview', async (_equipId: string) => {
      ...
      io.to(_channel).emit('star:preview:response', require)
    })
    ...
    socket.on('equip:star', async (_equipId: string) => {
      ...
      io.to(_channel).emit('equip:star:response', {
        gold,
        knb,
        daNangSao,
